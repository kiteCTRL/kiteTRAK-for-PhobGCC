...
void second_core() {
	...
	bool kiteTRAK_on = true;
	bool good_luck = false;
	int good_luck_time = 0;
	int kiteTRAK_mode = 0;
	int input_count = 0;
	int current_second = 0;
	int current_centisecond = 0;
	int current_second_disp = 0;
	int ipm_array[10] = {};
	char disp_text[8];
	uint16_t no_inputs_milli = 0;
  
    i2c_init(i2c0, 1000000);
    gpio_set_function(12, GPIO_FUNC_I2C); //SDA
    gpio_set_function(13, GPIO_FUNC_I2C); //SCL or SCK 
    gpio_pull_up(12);
    gpio_pull_up(13);

	// sleep_ms(250); // commenting out for now...
	...
	*/		
		// turn on/off
		if (_btn.Z && _btn.S && !oldButton.S) {
			kiteTRAK_mode = 0; // reset mode to boot text
			kiteTRAK_on = !kiteTRAK_on;
			display.clear();
			drawText(&display, font_16x32, "        ", 0, 0);
			display.sendBuffer();
		}
		oldButton.S = _btn.S;
		...
		if (kiteTRAK_on) {
			...
			//how many milliseconds since last input
			if (abs(_btn.Ax-127) < 20 && abs(_btn.Ay-127) < 20 && abs(_btn.Cx-127) < 20 && abs(_btn.Cy-127) < 20 && !_btn.A && !_btn.B && !_btn.X && !_btn.Y && !_btn.L && !_btn.R) {
				no_inputs_milli++;
			} else {
				no_inputs_milli = 0;
			}
			...
			if (_btn.Z && _btn.Dr && !oldButton.Dr) {
				display.invertDisplay();
			}
			oldButton.Dr = _btn.Dr;
		
			// kiteTRAK Mode 1: Time Tracking
			if (kiteTRAK_mode == 1) {
				int seconds = (millis() / 1000) % 60;
				int minutes = (millis() / (1000*60)) % 60;
				int hours = (millis() / (1000*60*60)) % 24;
				sprintf(disp_text, "%02d:%02d:%02d", hours, minutes, seconds);
			}

			// kiteTRAK Mode 2: IPM Tracking
			if (kiteTRAK_mode == 2) {
				int input_sum = 0;
				int second_counter = (millis() / 1000) % 10;
				if (current_second != second_counter) {
					ipm_array[current_second] = input_count;
					for (int i = 0; i < 10; i++) {
						input_sum += ipm_array[i];
					}
					int ipm = input_sum * 6;
					sprintf(disp_text, "%03d IPM", ipm);

					input_count = 0;
					current_second = second_counter;
				}

				if(_btn.A && !oldButton.A) { // A press
					input_count++;
				}
				oldButton.A = _btn.A;

				if(_btn.B && !oldButton.B) { // B press
					input_count++;
				}
				oldButton.B = _btn.B;

				if(_btn.X && !oldButton.X) { // X press
					input_count++;
				}
				oldButton.X = _btn.X;

				if(_btn.Y && !oldButton.Y) { // Y press
					input_count++;
				}
				oldButton.Y = _btn.Y;

				if(_btn.L && !oldButton.L) { // L press (dig)
					input_count++;
				}
				oldButton.L = _btn.L;

				if(_btn.R && !oldButton.R) { // R press (dig)
					input_count++;
				}
				oldButton.R = _btn.R;

				if(_btn.Z && !oldButton.Z) { // Z press
					input_count++;
				}
				oldButton.Z = _btn.Z;

				if((_btn.Ax-127 >= 23 && oldButton.Ax-127 < 23) || (abs(_btn.Ax-127) < 23 && abs(oldButton.Ax-127) >= 23)  || (_btn.Ax-127 <= -23 && oldButton.Ax-127 > -23)) { // A-stick X axis
					input_count++;
				}
				oldButton.Ax = _btn.Ax;

				if((_btn.Ay-127 >= 23 && oldButton.Ay-127 < 23) || (abs(_btn.Ay-127) < 23 && abs(oldButton.Ay-127) >= 23)  || (_btn.Ay-127 <= -23 && oldButton.Ay-127 > -23)) { // A-stick Y axis
					input_count++;
				}
				oldButton.Ay = _btn.Ay;

				if((_btn.Cx-127 >= 23 && oldButton.Cx-127 < 23) || (abs(_btn.Cx-127) < 23 && abs(oldButton.Cx-127) >= 23)  || (_btn.Cx-127 <= -23 && oldButton.Cx-127 > -23)) { // C-stick X axis
					input_count++;
				}
				oldButton.Cx = _btn.Cx;

				if((_btn.Cy-127 >= 23 && oldButton.Cy-127 < 23) || (abs(_btn.Cy-127) < 23 && abs(oldButton.Cy-127) >= 23)  || (_btn.Cy-127 <= -23 && oldButton.Cy-127 > -23)) { // C-stick X axis
					input_count++;
				}
				oldButton.Cy = _btn.Cy;
			}

			// kiteTRAK Mode 4: owo
			if (kiteTRAK_mode == 4) {
				int centisecond_counter = (millis() / 100) % 10;
				int second_counter = (millis() / 1000) % 10;

				if ((current_centisecond != centisecond_counter) && (no_inputs_milli > 1000)) {
					if (centisecond_counter % 5 == 0) {
						if ((second_counter == 0) && (centisecond_counter < 2)) {
							display.clear();
							if (!good_luck) {
								drawText(&display, font_16x32, "    -w- ", 0, 3);
							} else if (good_luck) {
								drawText(&display, font_16x32, "    >o< ", 0, 4);
								drawText(&display, font_16x32, "gl!     ", 0, 0);
							}
							display.sendBuffer();
						} else {
							display.clear();
							if (!good_luck) {
								drawText(&display, font_16x32, "    owo ", 0, 3);
							}
							if (good_luck) {
								drawText(&display, font_16x32, "    >o< ", 0, 4);
								drawText(&display, font_16x32, "gl!     ", 0, 0);
						...
						if ((second_counter == 0) && (centisecond_counter < 2)) {
							if (!good_luck) {
								drawText(&display, font_16x32, "    -w- ", 0, 0);
							} else if (good_luck) {
								drawText(&display, font_16x32, "gl! >o< ", 0, 0);
							}
							display.sendBuffer();
						} else {
							if (!good_luck) {
								drawText(&display, font_16x32, "    owo ", 0, 0);
							} else if (good_luck) {
								drawText(&display, font_16x32, "gl! >o< ", 0, 0);
							}
							display.sendBuffer();
					...
					current_centisecond = centisecond_counter;
				}
			}

			// kiteTRAK Mode 0: Boot Text
			if (kiteTRAK_mode == 0) {	
				sprintf(disp_text, "kiteTRAK");
			}

			//update display
			int second_counter = (millis() / 1000) % 10;
			if ((current_second_disp != second_counter) && (no_inputs_milli > 1000) && (kiteTRAK_mode != 4)) { // only update once per second when no inputs detected for at least 1 seconds
					display.clear();
					drawText(&display, font_16x32, disp_text, 0, 0);
					display.sendBuffer();
					current_second_disp = second_counter;
			}
		}
		//kiteTRAK end
